...
from .currency import _add_money, _fetch_wallet, _remove_money
...
ORDER BY sim LIMIT 1;
    """
    resp = await engine.fetch_one(query=query)
    if resp is None:
        await message.channel.send(
            "Waifu not found! You can add the waifu yourself, please join the support server! (`=support`) <a:thanks:699004469610020964>"
        )
    ...
    query = (
        ...
        .where(database.PurchasedWaifu.c.waifu_id == resp["id"])
        .where(database.PurchasedWaifu.c.guild == message.guild.id)
    )
    purchaser = await engine.fetch_one(query=query)
    ...
    if gender == "m":
        ...
        gender = "??Trap??"
    from_anime = resp["from_anime"]
    coins_req = resp["price"]
    gender = gender.lower()
    waifu_description = resp["description"]
    ...
    if resp["image_url"] is not None:
        images = resp["image_url"].split(",")
        embed.set_image(url=images[0])
    embed.add_field(name="From", value=resp["from_anime"])
    embed.add_field(name="Cost", value=f"{resp['price']} <:PIC:668725298388271105>")
...
def favorite(unfavorite=False):
    async def _favorite(client, message, *args):
        ...
        waifu = await engine.fetch_one(query)
        if waifu is None:
            await message.channel.send(
...
async def buy(client, message, *args):
    ...
    while not sure:
        try:
            ...
            handle_lock(f"{waifu_id}:{message.guild.id}", buy_lock, "REMOVE")
            return
    await _remove_money(message.author, cost)
    fetch_query = database.Member.select().where(
        database.Member.c.member == message.author.id
    )
    buyer = await engine.fetch_one(query=fetch_query)
    create_query = database.PurchasedWaifu.insert(None)
...
async def sell(client, message, *args):
    ...
    while not sure:
        try:
            ...
            return
    delete_query = (
        database.PurchasedWaifu.delete(None)
        .where(
    ...
    await engine.execute(delete_query)
    await _add_money(message.author, cost)
    handle_lock(f"{message.guild.id}:{message.author.id}", ug_lock, "REMOVE")
    handle_lock(f"{waifu_id}:{message.guild.id}", sell_lock, "REMOVE")
    await message.channel.send(
...
def trade(using_money=False):
    async def _trade(client, message, *args):
        ...
        try:
            ...
            sender_pwaifu = await engine.fetch_one(query=query)
            ...
            if using_money:
                if msg.content.isdigit():
                    ...
                    receiver_wallet = await _fetch_wallet(receiver)
                    if receiver_wallet - receiver_money < 0:
                        await message.channel.send(
                            f"You do not have enough money! <:Eww:575373991640956938>\nYou need {receiver_money-receiver_wallet} <:PIC:668725298388271105> more."
                        )
                        handle_lock(
                            f"{message.guild.id}:{receiver.id}", ug_lock, "REMOVE"
                        )
                        handle_lock(
                            f"{message.guild.id}:{sender.id}", ug_lock, "REMOVE"
                        )
                        return
                    await _remove_money(receiver, receiver_money)
                else:
                ...
                receiver_pwaifu = await engine.fetch_one(query=query)
...
"""
            ...
            if msg.content.lower() in ["yes", "y", "ye", "yeah", "yea", "yep"]:
                ...
                if using_money:
                    await _add_money(receiver, receiver_money)
                handle_lock(f"{message.guild.id}:{receiver.id}", ug_lock, "REMOVE")
                handle_lock(f"{message.guild.id}:{sender.id}", ug_lock, "REMOVE")
                return
            else:
        ...
        receiver_db = await engine.fetch_one(query=fetch_query)
        delete_query = (
            database.PurchasedWaifu.delete(None)
            .where(
                database.PurchasedWaifu.c.waifu_id
        ...
        if using_money:
            await _add_money(sender, receiver_money)
        else:
            delete_query = (
                database.PurchasedWaifu.delete(None)
                .where(
...
You have no rolls left! Rolls reset in {h:02d} hours {m:02d} minutes. You can donate to me and get more rolls!
    ...
    resp = await engine.fetch_one(query=count_query)
    ...
    if not purchaseable:
        return
    wallet = await _fetch_wallet(purchaser)
    if wallet - price < 0:
        ...
        return
    await _remove_money(purchaser, price)
    fetch_query = database.Member.select().where(
