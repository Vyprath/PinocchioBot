...
async def quiz_create(client, message, qtype, bonus_amt):
    ...
    success = await _remove_money(message.author, bonus_amt)
    if bonus_amt and not success:
        return await message.channel.send(
            "You don't have enough coins to start a quiz with that bonus!"
        )
    quizes.update(
        {
            quiz_id: {
...
async def quiz_join(client, message, qtype, maker):
    ...
    if quiz["started"]:
        return await message.channel.send(
            "This quiz has started! You can no longer join this quiz."
        )
    if message.author in quiz["members"]:
        return await message.channel.send("You have already joined the quiz!")
...
async def quiz_end(client, message, qtype):
    ...
    if awardable > 0 and ptsum > 0:
        ...
        for m in firstplace:
            await _add_money(m, firstaward)
            awarded.update({m: firstaward})
        for m in secondplace:
            await _add_money(m, secondaward)
            awarded.update({m: secondaward})
        for m in thirdplace:
            await _add_money(m, thirdaward)
...
async def text_quiz_start(client, message, difficulty):
    ...
    if difficulty.lower() not in ["easy", "medium", "hard", ""]:
        return await message.channel.send(
            "Difficulty is optional, but has to be easy/medium/hard."
        )
    quiz = quizes[quiz_id]
    if quiz["started"]:
        return await message.channel.send("This quiz has started!")
    quiz["started"] = True
...
async def fetch_waifu_questions(num_q, num_choices):
    ...
    results = await engine.fetch_all(query=question_query)
    incorrect_answer_query = (
        database.Waifu.select()
        .order_by(func.random())
        .where(~database.Waifu.c.name.in_([i[database.Waifu.c.name] for i in results]))
        .limit(num_q * (10 + num_choices))
    )
    random_answers = await engine.fetch_all(query=incorrect_answer_query)
    for q in results:
...
async def waifu_quiz_start(client, message, difficulty):
    ...
    members = quiz["members"]
    numrounds = min(5, 50 // len(members))
    tmpmsg = await message.channel.send("Preparing quiz... Please wait!")
    num_choices = 4
    if difficulty == "easy":
        num_choices = 3
...
async def waifu_quiz(client, message, *args):
    ...
    if arg == "create":
        ...
        return await waifu_quiz_start(
            client, message, args[1].lower() if len(args) == 2 else ""
        )
