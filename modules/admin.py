...
async def purge_all_waifus(client, message, *args):
    ...
    while not sure:
        try:
            ...
            return
    engine = await database.prepare_engine()
    delete_query = database.PurchasedWaifu.delete(None).where(
        database.PurchasedWaifu.c.guild == message.guild.id
    )
    await engine.execute(delete_query)
    await message.channel.send(":skull_crossbones: Removed waifus from everyone!")
...
async def purge_waifus(client, message, *args):
    ...
    if len(message.mentions) != 1:
        return await message.channel.send(
            f"Usage: `{PREFIX}forcedivorcewaifus <@user>`: Remove all waifus from a user."
        )
    ...
    engine = await database.prepare_engine()
    delete_query = database.PurchasedWaifu.delete(None).where(
        database.PurchasedWaifu.c.member == user.id
    )
    await engine.execute(delete_query)
    await message.channel.send(f":skull_crossbones: Removed waifus from {str(user)}!")
