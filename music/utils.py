...
regex_str = r"^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$"
...
class MusicInfo:
    ...
    def process_url(self):
        ...
        self.thumbnail = (
            self.raw_info["thumbnail"] if "thumbnail" in self.raw_info else None
        )
    ...
    def get_embed(self):
        embed = discord.Embed(
            ...
            url=self.video_url,
            color=self.requested_by.colour,
        )
        mins = self.duration // 60
        secs = self.duration - mins * 60
        embed.add_field(
            name="Average Audio Bitrate", value="{0} KBit/s".format(self.abr)
        )
        if self.asr:
            embed.add_field(name="Audio Sampling Rate", value="{0} Hz".format(self.asr))
        if self.acodec:
            embed.add_field(name="Audio Codec", value=self.acodec)
        embed.add_field(name="Duration", value="{0:>02d}:{1:>02d}".format(mins, secs))
        if self.time_left:
        ...
        embed.set_footer(
            text="Requested by: {0}".format(self.requested_by.name),
